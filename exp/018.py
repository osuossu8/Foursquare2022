...
train = train[categorical_features + ['text'] + [CFG.target, "num_target", "id"] + [f"target_{i}" for i in range(10)] + [f"near_id_{i}" for i in range(CFG.n_neighbors)]]
...
params = {
    'objective': "MultiRMSE", # "MultiLogloss",  # "F1", # "MultiRMSE", # "MultiClass", # "Logloss",
    'learning_rate': 0.2,
    #'reg_alpha': 0.1,
    'reg_lambda': 0.1,
    'random_state': 42,
...
oof, models = fit_cat(train[features+categorical_features], train[[f"target_{i}" for i in range(10)]],
...
train[[f"oof_{i}" for i in range(10)]] = oof > 0.5
