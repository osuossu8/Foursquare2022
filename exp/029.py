...
train[[f'use_vector1_{i}' for i in range(512)]] = np.stack(train['id'].map(id_2_text_use_vector))
train[[f'use_vector2_{i}' for i in range(512)]] = np.stack(train['match_id'].map(id_2_text_use_vector))
...
for test_path in tqdm([
    ...
    test[[f'use_vector1_{i}' for i in range(512)]] = np.stack(test['id'].map(id_2_text_use_vector))
    test[[f'use_vector2_{i}' for i in range(512)]] = np.stack(test['match_id'].map(id_2_text_use_vector))
