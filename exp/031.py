...
TRAIN_FEATURES = ['kdist',
                ...
                'text_1',
                'text_2',
] + [f'{v}_1' for v in vec_columns] + [f'{v}_2' for v in vec_columns]
...
for v in vec_columns:
    id_2_cat[v] = {k:v for k, v in zip(data['id'].values, data[v].fillna('nan').values)}
...
train['oof'] = np.load('output/027/'+'oof.npy')
...
for v in vec_columns:
    test[f'{v}_1'] = test['id'].map(id_2_cat[v])
    test[f'{v}_2'] = test['match_id'].map(id_2_cat[v])
