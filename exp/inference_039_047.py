...
models = [unpickle('output/047/'+f'cat_fold{i}.pkl') for i in range(CFG.n_splits)]
...
for test_path in tqdm([
    ...
    res_df.append(test[test['pred'] > 0.5][['id', 'match_id', 'pred']])
...
test.to_csv('upload/valid_preds_047.csv', index=False)
...
models = [unpickle('output/039/'+f'cat_fold{i}.pkl') for i in range(CFG.n_splits)]
...
print(test[['id', 'match_id', 'pred']])
test = test[test['pred'] > 0.5][['id', 'match_id', 'pred']]

test.to_csv('upload/valid_preds_039.csv', index=False)
