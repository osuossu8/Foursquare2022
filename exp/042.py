...
id_2_name = {k:v for k, v in zip(data['id'].values, data['name'].fillna('').values)}

del data;gc.collect()

NAMES_EMBEDDINGS_USE = np.load('embeddings/NAMES_EMBEDDINGS_USE.npy')
NAMES_EMBEDDINGS_MPNET = np.load('embeddings/NAMES_EMBEDDINGS_MPNET.npy')
...
with open('embeddings/name2names_embedding_idx_dict.pkl', 'rb') as f:
...
train = add_name_distance_features(train)
...
test = add_name_distance_features(test)
