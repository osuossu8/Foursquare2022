...
del id_2_w2v_vec_train, w2v_sim; gc.collect()
...
for nv1, nv2 in tqdm(zip(train['id'].map(id_2_text_tfidf_bm25_svd_vector), train['match_id'].map(id_2_text_tfidf_bm25_svd_vector))):
    bm25_sim.append(cosine_similarity(nv1, nv2))
train['text_sim_bm25_svd'] = bm25_sim
del id_2_text_tfidf_bm25_svd_vector, bm25_sim; gc.collect()
...
print(train[['text_sim_bm25_svd', 'name_longest_substr_ratio', 'address_longest_substr_ratio', 'categories_longest_substr_ratio', 'text_sim_w2v', 'name_sim_use', 'categories_sim_use', 'category_venn']].head())
...
def fit_xgb(X, y, params=None,
    ...
    for i in tqdm(range(CFG.n_splits)):
        ...
        if model_dir is None:
            ...
            val_set = xgboost.DMatrix(data = X_valid, label = y_valid)
        ...
        pred = model.predict(xgboost.DMatrix(X_valid))
        oof[val_idx] = pred
        models.append(model)

        file = OUTPUT_DIR+f'xgb_fold{i}.pkl'
...
oof, models = fit_xgb(train[TRAIN_FEATURES], train["label"].astype(int),
...
models = [unpickle(OUTPUT_DIR+f'xgb_fold{i}.pkl') for i in range(CFG.n_splits)]
