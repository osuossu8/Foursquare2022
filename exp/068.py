...
print('load data')
train = pd.read_csv('input/train_data_candidate_25_faiss_v2_1.csv')
train = reduce_mem_usage(train)
print(train['label'].value_counts())
train2 = pd.read_csv('input/train_data_candidate_25_faiss_v2_2.csv')
train2 = reduce_mem_usage(train2)
print(train2['label'].value_counts())
train3 = pd.read_csv('input/train_data_candidate_25_faiss_v2_3.csv')
train3 = reduce_mem_usage(train3)
print(train3['label'].value_counts())
train4 = pd.read_csv('input/train_data_candidate_25_faiss_v2_4.csv')
train4 = reduce_mem_usage(train4)
print(train4['label'].value_counts())
train5 = pd.read_csv('input/train_data_candidate_25_faiss_v2_5.csv')
...
#id_2_text_tfidf_bm25_svd_vector = unpickle(f'features/id_2_text_tfidf_bm25_svd_vector_64d_valid_ids.pkl')
#id_2_text_mpnet_vector = unpickle(f'features/id_2_text_mpnet_vector_valid_ids.pkl')

models = [unpickle(OUTPUT_DIR+f'cat_fold{i}.pkl') for i in range(CFG.n_splits)]

res_df = []
for test_path in tqdm([
    'input/valid_data_candidate_25_faiss_v2_1.csv',
    'input/valid_data_candidate_25_faiss_v2_2.csv',
    'input/valid_data_candidate_25_faiss_v2_3.csv',
    'input/valid_data_candidate_25_faiss_v2_4.csv',
    'input/valid_data_candidate_25_faiss_v2_5.csv'
    ]):
    ...
    test['latitude_round_1'] = test['latitude_1'].round(1)
    test['longitude_round_1'] = test['longitude_1'].round(1)
    test['latitude_round_2'] = test['latitude_2'].round(1)
    test['longitude_round_2'] = test['longitude_2'].round(1)
    test = add_haversine_distance(test)
    del test['latitude_1'], test['latitude_2'], test['longitude_1'], test['longitude_2']; gc.collect()
    """
    sim = []
    for nv1, nv2 in tqdm(zip(test['id'].map(id_2_w2v_vec_valid), test['match_id'].map(id_2_w2v_vec_valid))):
        sim.append(cosine_similarity(nv1, nv2))
    test['text_sim_w2v'] = sim

    sim = []
    for nv1, nv2 in tqdm(zip(test['id'].map(id_2_text_tfidf_bm25_svd_vector), test['match_id'].map(id_2_text_tfidf_bm25_svd_vector))):
        sim.append(cosine_similarity(nv1, nv2))
    test['text_sim_bm25_svd'] = sim

    sim = []
    for nv1, nv2 in tqdm(zip(test['id'].map(id_2_text_mpnet_vector), test['match_id'].map(id_2_text_mpnet_vector))):
        sim.append(cosine_similarity(nv1, nv2))
    test['text_sim_mpnet'] = sim
    del sim; gc.collect()
    ...
    test['number_1'] = test['address_1'].str.extract('(\d+)')
