...
tfidf_svd_emb_features = unpickle('features/tfidf_svd_embedding_features.pkl')
tfidf_svd_emb_features = pd.DataFrame(tfidf_svd_emb_features, columns=[f'tfidf_svd_embs_{i+1}' for i in range(160)])
features += [f'tfidf_svd_embs_{i+1}' for i in range(160)]

print(tfidf_svd_emb_features.shape)


train = train[[CFG.target, "num_target", "id"] + [f"target_{i}" for i in range(10)] + [f"near_id_{i}" for i in range(CFG.n_neighbors)]]

train = pd.concat([train, distance_features, tfidf_svd_emb_features], 1)
