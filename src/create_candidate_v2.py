...
def add_neighbor_features(df):
    ...
    for country, country_df in tqdm(df.groupby("country")):
        ...
        lats = country_df['latitude'].values
        lons = country_df['longitude'].values
        for i in range(min(len(country_df), CFG.n_neighbors)):
            ...
            country_df[f"near_latitude_{i}"] = lats[nears[:, i]]
            country_df[f"near_longitude_{i}"] = lons[nears[:, i]]
            for c in columns:
        ...
        for i in range(min(len(country_df), CFG.n_neighbors), CFG.n_neighbors):
            ...
            country_df[f"near_latitude_{i}"] = np.nan
            country_df[f"near_longitude_{i}"] = np.nan
            for c in columns:
...
train.to_csv('input/train_with_near_candidate_target_v2.csv', index=False)
