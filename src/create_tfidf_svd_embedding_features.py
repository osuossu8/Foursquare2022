...
for r in tqdm(range(len(train))):
    ...
    for i in range(CFG.n_neighbors):
        if row[f'near_id_{i}'] == 'unknown':
            emb = np.zeros(16)
        else:
            emb = text_embeddings[id_idx_map[row[f'near_id_{i}']]]
        vec.append(emb)
    vec_all.append(np.concatenate(vec))

print(len(vec_all))
print(np.array(vec_all).shape)

to_pickle('features/tfidf_svd_embedding_features.pkl', np.array(vec_all))
