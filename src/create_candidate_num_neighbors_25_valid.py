...
logger = init_logger(log_file='log/' + f"create_candidate_25_valid.log")
...
def recall_knn(df, Neighbors = 10):
    logger.info('Start knn grouped by country')
    train_df_country = []
...
for k in range(1, num_split + 1):
    ...
    cur_data.to_csv('input/valid_data_candidate_25_%s.csv' % k, index = False)
    start_row = end_row
    count += len(cur_data)
